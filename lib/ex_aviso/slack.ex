defmodule ExAviso.Slack do
  ...
  defp connect_websocket(%{domain: domain, path: path}) do
    ...
    children = [
      worker(ExAviso.SlackNotifier, [socket])
    ]
  ...
  def handle_cast({:notification, m}, t) do
    GenServer.cast(ExAviso.SlackNotifier, {:notification, m})
    {:noreply, t}
  end
  ...
  def get_option(line, default) do
    String.split(line, " ")
    # オプションとして記載されているものの取得
    |> Enum.map(&Regex.named_captures(~r/(?<name>.*)=(?<value>.*)/, &1))
    # nilを削除
    |> Enum.filter(&(&1 != nil))
    # name = valueでオプションを指定しているので%{name => value}でmapを作成
